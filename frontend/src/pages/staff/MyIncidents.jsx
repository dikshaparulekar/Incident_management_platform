...
export default function StaffMyIncidents() {
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500">{data.total} incidents assigned to you</p>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700/50">
              <tr>
                ...
                <th className="table-th">Quick Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {loading ? Array(5).fill(0).map((_, i) => (
                <tr key={i}><td colSpan={6} className="table-td"><div className="skeleton h-4 w-full" /></td></tr>
                ...
                <tr key={inc.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                  ...
                  <td className="table-td max-w-xs">
                    <button onClick={() => navigate(`/staff/incidents/${inc.id}`)} className="font-medium hover:text-primary-600 text-left line-clamp-1">{inc.title}</button>
                  </td>
                  ...
                  <td className="table-td">
                    <div className="flex gap-1">
                      {inc.status === 'OPEN' && <button onClick={() => quickUpdate(inc.id, 'IN_PROGRESS')} className="btn-secondary text-xs px-2 py-1">Start</button>}
                      {inc.status === 'IN_PROGRESS' && <button onClick={() => quickUpdate(inc.id, 'RESOLVED')} className="btn-secondary text-xs px-2 py-1 text-green-600">Resolve</button>}
                      <button onClick={() => navigate(`/staff/incidents/${inc.id}`)} className="btn-ghost text-xs px-2 py-1">View</button>
                    </div>
                  </td>
                </tr>
              ))}
              {!loading && !data.incidents.length && (
                <tr><td colSpan={6} className="table-td text-center text-gray-500 py-8">No incidents assigned to you</td></tr>
              )}
            </tbody>
          </table>
        </div>
